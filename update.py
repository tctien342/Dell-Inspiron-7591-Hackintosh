...
def zip_folder(folders):
	...
	for folder in folders:
		...
		sh('mkdir -p Builds/{}/EFI/{} && cp -R {}/* Builds/{}/EFI/{}/'.format(folder.name, folder.name ,folder.name ,folder.name,folder.name))
		sh('mkdir -p Builds/{}/EFI/BOOT && cp -R BOOT/{}/* Builds/{}/EFI/BOOT/'.format(folder.name,folder.name,folder.name))
